...
class FlowPolicy(nn.Module):
    ...
    def _apply_ellipse_constraints(self, opti, X_var, X_index_selector=None):
        ...
        # apply constraints for each horizon step (except the first and last)
        for i in range(self.horizon - 2):
            if X_index_selector is None:
                x_pos = X_var[
                    -1,
                    2 * self.action_dim + i * (self.action_dim + self.state_dim) + 1,
                ]
                y_pos = X_var[
                    -1,
                    2 * self.action_dim + i * (self.action_dim + self.state_dim),
                ]
    ...
    def _apply_dynamics_constraints(self, opti, X_var, X_index_selector=None):
        ...
        for i in range(self.horizon - 3):
            if X_index_selector is None:
                ...
                state = X_var[-1, state_index : state_index + self.state_dim]
                next_state_index = state_index + (self.action_dim + self.state_dim)
                next_state = X_var[
                    -1, next_state_index : next_state_index + self.state_dim
                ]
                action_index = state_index - self.action_dim
                action = X_var[-1, action_index : action_index + self.action_dim]
            elif X_index_selector == "projection":
                state_index = 2 * self.action_dim + i * (
                    self.action_dim + self.state_dim
                )
        ...
        if X_index_selector is None:
            ...
            sHm1_index = 2 * self.action_dim + (self.horizon - 3) * (
                self.action_dim + self.state_dim
            )
            sHm1 = X_var[-1, sHm1_index : sHm1_index + self.state_dim]
            aHm1_index = sHm1_index - self.action_dim
            aHm1 = X_var[-1, aHm1_index : aHm1_index + self.action_dim]
        elif X_index_selector == "projection":
            s1_index = 2 * self.action_dim
            ...
            sHm1 = X_var[sHm1_index : sHm1_index + self.state_dim]
            aHm1_index = sHm1_index - self.action_dim
            aHm1 = X_var[aHm1_index : aHm1_index + self.action_dim]
        else:
            raise NotImplementedError
        ...
        opti.subject_to(
            cs.mtimes(self.dynamics_model["A"], cs.reshape(sHm1, -1, 1))
            + cs.mtimes(self.dynamics_model["B"], cs.reshape(aHm1, -1, 1))
            + cs.reshape(self.dynamics_model["c"], -1, 1)
            == cs.reshape(self.oc_sH_param, -1, 1)
        )
    ...
    def _generate_distance_objective(self, opti, X_var, X_index_selector=None):
        ...
        self.oc_distance_objective = cs.MX(0.0)
        ...
        for i in range(self.horizon - 3):
            ...
            self.oc_distance_objective += cs.sumsqr(state[0:2] - next_state[0:2])
        ...
        self.oc_distance_objective += cs.sumsqr(
            s1[0:2] - self.oc_s0_param[0:2].T
        ) + cs.sumsqr(sHm1[0:2] - self.oc_sH_param[0:2].T)

        self.oc_distance_objective = -1.0 * self.oc_distance_objective
    ...
    def projection_formulate(
        ...
        self.oc_dof = (
            self.horizon * (self.state_dim + self.action_dim) - self.state_dim * 2
        )

        self.oc_s0_param = self.oc_cs_opti.parameter(1, self.state_dim)
        self.oc_sH_param = self.oc_cs_opti.parameter(1, self.state_dim)
        ...
        if constraint == "ellipses":
            self._apply_ellipse_constraints(
                self.oc_cs_opti, self.oc_X_single, X_index_selector="projection"
            )
        elif constraint == "ellipses_and_dynamics":
            self._apply_ellipse_constraints(
                self.oc_cs_opti, self.oc_X_single, X_index_selector="projection"
            )
    ...
    def _build_constraint_funs(self):
        ...
        constraint = getattr(self.cfg, "constraint", "")

        x_var = cs.SX.sym("x_var", self.oc_dof)
        s0 = cs.SX.sym("s0", self.state_dim)
        sH = cs.SX.sym("sH", self.state_dim)

        ineq_constraints = []
        eq_constraints = []

        if constraint in ["ellipses", "ellipses_and_dynamics"]:
            obs_norm_min_0 = float(self.obs_norm_mins[0])
            obs_norm_max_0 = float(self.obs_norm_maxs[0])
            obs_norm_min_1 = float(self.obs_norm_mins[1])
            obs_norm_max_1 = float(self.obs_norm_maxs[1])
            ...
            yb = 2.0 * 0.8 / (obs_norm_max_0 - obs_norm_min_0)
            off_xb = (
                2.0 * (4.8 - obs_norm_min_1) / (obs_norm_max_1 - obs_norm_min_1) - 1.0
            )
            off_yb = (
                2.0 * (1.5 - obs_norm_min_0) / (obs_norm_max_0 - obs_norm_min_0) - 1.0
            )

            ellipse_margin = float(self.cfg.ellipse_margin)

            for i in range(self.horizon - 2):
                state_index = 2 * self.action_dim + i * (
                    self.action_dim + self.state_dim
                )
                y_pos = x_var[state_index]
                x_pos = x_var[state_index + 1]

                first_obstacle = ellipse_margin - (
                    (1.0 / xa * (x_pos - off_xa)) ** 2
                    + (1.0 / ya * (y_pos - off_ya)) ** 2
                    - 1.0
                )
                second_obstacle = ellipse_margin - (
                    (1.0 / xb * (x_pos - off_xb)) ** 4
                    + (1.0 / yb * (y_pos - off_yb)) ** 4
                    - 1.0
                )

                ineq_constraints.append(first_obstacle)
                ineq_constraints.append(second_obstacle)

        if constraint == "ellipses_and_dynamics":
            assert (
                self.dynamics_model is not None
            ), "dynamics model must be loaded for dynamics constraints"

            A = cs.DM(self.dynamics_model["A"])
            B = cs.DM(self.dynamics_model["B"])
            c_vec = cs.DM(self.dynamics_model["c"]).reshape((self.state_dim, 1))
            ...
            for i in range(self.horizon - 3):
                ...
                state_col = cs.reshape(state, (self.state_dim, 1))
                next_state_col = cs.reshape(next_state, (self.state_dim, 1))
                action_col = cs.reshape(action, (self.action_dim, 1))

                dyn_residual = (
                    cs.mtimes(A, state_col)
                    + cs.mtimes(B, action_col)
                    + c_vec
                    - next_state_col
                )

                for j in range(self.state_dim):
                    eq_constraints.append(dyn_residual[j])

            s1_index = 2 * self.action_dim
            s1 = x_var[s1_index : s1_index + self.state_dim]
            a0 = x_var[0 : self.action_dim]
            ...
            s0_col = cs.reshape(s0, (self.state_dim, 1))
            sH_col = cs.reshape(sH, (self.state_dim, 1))
            s1_col = cs.reshape(s1, (self.state_dim, 1))
            sHm1_col = cs.reshape(sHm1, (self.state_dim, 1))
            a0_col = cs.reshape(a0, (self.action_dim, 1))
            aHm1_col = cs.reshape(aHm1, (self.action_dim, 1))

            initial_dyn_residual = (
                cs.mtimes(A, s0_col) + cs.mtimes(B, a0_col) + c_vec - s1_col
            )
            terminal_dyn_residual = (
                cs.mtimes(A, sHm1_col) + cs.mtimes(B, aHm1_col) + c_vec - sH_col
            )

            for j in range(self.state_dim):
                eq_constraints.append(initial_dyn_residual[j])
                eq_constraints.append(terminal_dyn_residual[j])

            action_lower = -np.ones(self.action_dim)
            action_upper = np.ones(self.action_dim)

            for j in range(self.action_dim):
                ineq_constraints.append(action_lower[j] - x_var[j])
                ineq_constraints.append(x_var[j] - action_upper[j])

            for i in range(self.horizon - 1):
                action_index = self.action_dim + i * (self.action_dim + self.state_dim)
                for j in range(self.action_dim):
                    action_val = x_var[action_index + j]
                    ineq_constraints.append(action_lower[j] - action_val)
                    ineq_constraints.append(action_val - action_upper[j])
        if constraint not in ["", "ellipses", "ellipses_and_dynamics"]:
            raise ValueError(f"Unsupported constraint: {constraint}")

        self._al_m_ineq = len(ineq_constraints)
        self._al_m_eq = len(eq_constraints)

        if self._al_m_ineq > 0:
            c_expr = cs.vertcat(*ineq_constraints)
            self._al_ineq_fun = cs.Function("al_ineq", [x_var, s0, sH], [c_expr])
            self._al_Jc_fun = cs.Function(
                "al_Jc", [x_var, s0, sH], [cs.jacobian(c_expr, x_var)]
            )
        else:
            self._al_ineq_fun = None
            self._al_Jc_fun = None

        if self._al_m_eq > 0:
            h_expr = cs.vertcat(*eq_constraints)
            self._al_eq_fun = cs.Function("al_eq", [x_var, s0, sH], [h_expr])
            self._al_Jh_fun = cs.Function(
                "al_Jh", [x_var, s0, sH], [cs.jacobian(h_expr, x_var)]
            )
        else:
            self._al_eq_fun = None
            self._al_Jh_fun = None
    ...
    def warmstart(self, conditions):
        ...
        best_sH = best_x_chain[0, -self.state_dim :]
        best_dof_chain_part_1 = best_x_chain[:, : self.action_dim]
        best_dof_chain_part_2 = best_x_chain[
            :,
            self.action_dim
            + self.state_dim : self.horizon * (self.action_dim + self.state_dim)
            - self.state_dim,
        ]
        ...
        best_sH_np = to_np(best_sH)
        best_dof_chain_np = to_np(best_dof_chain)

        return best_s0_np, best_sH_np, best_dof_chain_np
    ...
    def projection_forward(self, conditions, batch_size=1):
        ...
        best_s0_np, best_sH_np, best_dof_chain_np = self.warmstart(conditions)
        best_final_dof = best_dof_chain_np[-1, :]

        t_start = time.time()

        self.oc_cs_opti.set_value(self.oc_s0_param, best_s0_np)
        self.oc_cs_opti.set_value(self.oc_sH_param, best_sH_np)
        ...
        for k in range(self.oc_N_steps):
            ...
            x_k = np.concatenate(
                [
                    ...
                    x_k_full[self.action_dim + self.state_dim : -self.state_dim],
                ],
            )

            v_k = self.constrained_flow_fn(x_k, t_k, best_s0_np, best_sH_np)
        ...
        optimized_x_chain = np.concatenate(
            [
                ...
                optimized_dof[:, self.action_dim :],
                np.tile(best_sH_np, (self.oc_N_steps + 1, 1)),
            ],
            axis=1,
        )
        ...
        optimized_final_x = np.append(optimized_final_x, best_sH_np)
    ...
    def projection_relaxed_forward(self, conditions, batch_size=1):
        ...
        for k in range(self.oc_N_steps):
            ...
            al_steps = int(getattr(self.cfg, "projection_relaxed_steps", 10))
            al_lr = float(getattr(self.cfg, "projection_relaxed_lr", 0.001))

            x_var = np.array(x_next_ref, dtype=np.float32).reshape(-1)
            s0_val = np.array(best_s0_np, dtype=np.float32).reshape(-1)
            sH_val = np.array(best_sH_np, dtype=np.float32).reshape(-1)
            ...
            if k == 0:
                ...
                al_rho = float(getattr(self.cfg, "projection_relaxed_rho", 2.0))

            for _ in range(al_steps):
                c_val = (
                    np.array(self._al_ineq_fun(x_var, s0_val, sH_val)).reshape(-1)
                    if m_ineq > 0
                    else np.zeros(0)
                )
                h_val = (
                    np.array(self._al_eq_fun(x_var, s0_val, sH_val)).reshape(-1)
                    if m_eq > 0
                    else np.zeros(0)
                )

                Jc = (
                    np.array(self._al_Jc_fun(x_var, s0_val, sH_val))
                    if m_ineq > 0
                    else np.zeros((0, x_var.size))
                )
                Jh = (
                    np.array(self._al_Jh_fun(x_var, s0_val, sH_val))
                    if m_eq > 0
                    else np.zeros((0, x_var.size))
                )
            ...
            if control_flag:
                x_terminal_predicted_ref = (
                    ...
                    * self.constrained_flow_fn(
                        x_next_ref, t_k + dt, best_s0_np, best_sH_np
                    ).T
                )
    ...
    def hardflow_new_forward(self, conditions, batch_size=1):
        ...
        PyTorch directly instead of going through the l4casadi bridge. The
        per-step QP formulation (`hardflow_formulate`) is unchanged because it
        only references algebraic constraints, so the optimization itself does
        not need the neural network.
        """
        assert batch_size == 1, "batch_size must be 1 for optimal control"
        assert (
    ...
    def oc_flow_forward(
        ...
        self.oc_N_steps = self.cfg.ode_t_steps
        self.oc_dof = (
            self.horizon * (self.state_dim + self.action_dim) - 2 * self.state_dim
        )
        ...
        best_sH = torch.tensor(best_sH_np, device=device, dtype=torch.float32)
        if best_s0.dim() == 1:
            best_s0 = best_s0.unsqueeze(0)
        if best_sH.dim() == 1:
            best_sH = best_sH.unsqueeze(0)
        ...
        def rollout(U):
            ...
            for k in range(self.oc_N_steps):
                ...
                v_k = self.constrained_flow_fn_torch(
                    x.unsqueeze(0), t_k, best_s0, best_sH
                ).squeeze(0)
                v_k = v_k + U[k]
                x = x + v_k * dt
                X.append(x)
            ...
            for j in reversed(range(N)):
                ...
                def F_j(x_flat):
                    ...
                    f = self.constrained_flow_fn_torch(
                        x.unsqueeze(0), t_j, best_s0, best_sH
                    ).squeeze(0)
    ...
    def constrained_flow_fn_torch(self, dof, t, s0, sH):

        device = dof.device

        assert dof.shape == (1, self.oc_dof)

        full_traj = torch.zeros(
            1, self.horizon * (self.action_dim + self.state_dim), device=device
        )

        full_traj[:, : self.action_dim] = dof[:, : self.action_dim]
        full_traj[:, self.action_dim : self.action_dim + self.state_dim] = s0
        full_traj[:, self.action_dim + self.state_dim : -self.state_dim] = dof[
            :, self.action_dim :
        ]
        full_traj[:, -self.state_dim :] = sH

        full_traj = full_traj.reshape(1, self.horizon, self.action_dim + self.state_dim)

        if isinstance(t, (float, int)):
            t = torch.tensor(t, device=device, dtype=full_traj.dtype)
        if t.dim() == 0:
            t = t.unsqueeze(0).expand(1)

        flow_output = self.flow_model(full_traj, t)

        flow_output_flat = flow_output.reshape(1, -1)

        dof_flow = torch.zeros(1, dof.shape[1], device=device)
        dof_flow[:, : self.action_dim] = flow_output_flat[:, : self.action_dim]
        dof_flow[:, self.action_dim :] = flow_output_flat[
            :, self.action_dim + self.state_dim : -self.state_dim
        ]
